...
const { data: servicesResponse } = await useAsyncData('services-list', () =>
  find('services', {
    filters: {
      is_an_activity:{
        $ne: true // Filtre : tout ce qui n'est pas "true"
      }
    },
    populate: {
      blocks: { populate: '*'}
...
const serviceItems = computed(() => (servicesResponse.value?.data as any[]) || [])
      ...
      <div class="max-w-6xl mx-auto px-4 lg:px-0">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <NuxtLink 
            ...
            class="group bg-gray-50 border border-gray-100 rounded-[10px] p-8 transition-all transform flex flex-col h-full"
          >
            <h2 class="text-2xl font-bold text-gray-900 mb-4 transition-colors">
              {{ service.name }}
            </h2>
            <p class="text-gray-600 mb-8 flex-grow">
              {{ service.short_description || 'Découvrez ce service en détail...' }}
            </p>
            
            <div class="mt-auto">
              <span class="inline-block rounded-[10px] px-5 py-3 bg-[#78E0AF] text-black font-semibold items-center gap-2 group-hover:bg-indigo-600/5 transition-all transform w-fit">
